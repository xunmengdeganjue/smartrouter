...
dowork()
{
	...
	if [ -f xml2func ];then
		if [ -f tr181.xml ];then
			...
			if [ ! $? -eq 0 ];then
				...
				sudo cp tr69_handler_table.h ${ONEAGENT_HOME}/
				sudo cp tr69_handler_ext.h ${ONEAGENT_HOME}/
			fi
		else	
	...
	if [ -f trfcmo ];then
		...
		if [ $? -eq 0 ];then
			...
			sudo cp ./tr69_handler.c ${ONEAGENT_HOME}/tr69_handler.h
		else 
	...
	if [ -f truci ];then
		...
		if [ $? -eq 0 ];then
			...
			sudo cp ./tr_uciconfig.h ${ONEAGENT_HOME}/	
		else 
